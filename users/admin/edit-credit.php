...
.error {
  ...
  if(isset($_POST['submit'])){
		...
		if($credit ==''){
			...
			$error[] = 'Please enter the store credit.';
...
$message .= "<tbody>
       ...
       <label>Your store credit has been updated:</label>
	   <label>Username: ".$username."</label> <BR />
	   <label>Your new amount is: ". $_POST['credit']."</label> <BR />
  ...
  // $mail->send();
	...
	if(isset($error)){
		...
		foreach($error as $error){
			...
			$stmt = $dbconn->prepare('SELECT userID, username, email, money FROM users WHERE username = :username') ;
...
<form action='' method='post' align = "center">
		...
		<p><label>Credit</label><br />

		<input type='int' name='credit' required onkeypress="return isNumber(event)" value='<?php echo $row['money'];?>'> </p>
    ...
    <script>
//check that only numbers are being inputted into credit field
